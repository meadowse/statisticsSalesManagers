...
def successManagers(request):
    ...
    with firebirdsql.connect(host=host, database=database, user=user, password=password, charset=charset) as con:
        ...
        WHERE T3.F27 = 3
        GROUP BY T3.F4886, T3.F5383, T3.ID
...
def projectManagers(request):
    with firebirdsql.connect(
        ...
        cur = con.cursor()
        sql = """SELECT T212.ID AS Contract_ID,
                 ...
                 FROM T212
                          ...
                          LEFT JOIN T3 T3_2 ON T212.F4546 = T3_2.ID -- Рук отдела
                 ...
                 WHERE T212.F4544 <> 'Аннулировано' AND T212.F4544 <> 'Успех' AND T212.F4544 <> 'Отменена'"""  # F4648 - путь, F4538 - номер договора, F4544 - стадия, F4946 - адрес, F4948 - направление, F4566 - дата окончания
        cur.execute(sql)
        result = cur.fetchall()
        # Преобразование результата в список словарей
